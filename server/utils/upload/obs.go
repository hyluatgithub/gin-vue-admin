...
func NewHuaWeiObsClient() (client *obs.ObsClient, err error) {
	return obs.New(global.ECOVACS_CONFIG.HuaWeiObs.AccessKey, global.ECOVACS_CONFIG.HuaWeiObs.SecretKey, global.ECOVACS_CONFIG.HuaWeiObs.Endpoint)
}
...
func (o *Obs) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	input := &obs.PutObjectInput{
		PutObjectBasicInput: obs.PutObjectBasicInput{
			ObjectOperationInput: obs.ObjectOperationInput{
				Bucket: global.ECOVACS_CONFIG.HuaWeiObs.Bucket,
				Key:    filename,
			},
	...
	filepath := global.ECOVACS_CONFIG.HuaWeiObs.Path + "/" + filename
